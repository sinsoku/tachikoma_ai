  ...
  describe Repository do
    ...
    describe '#compare' do
      ...
      context 'Response: 200 OK ' do
        ...
        context 'the end tag not exists' do
          before do
            stub_request(:get, repository.send(:api_tags_url))
              .to_return(status: 200, body: '[{"ref":"refs/tags/v0.1.0"}]')
          end
          subject { repository.compare('0.1.0', '0.2.0') }
          it { is_expected.to eq 'https://github.com/sinsoku/tachikoma_ai/compare/v0.1.0...master' }
        end

        context 'both tags not exist' do
          before do
            stub_request(:get, repository.send(:api_tags_url))
              .to_return(status: 200, body: '[]')
          end
          subject { repository.compare('0.1.0', '0.2.0') }
          it { is_expected.to eq 'https://github.com/sinsoku/tachikoma_ai (tags not found)' }
        end
      end
